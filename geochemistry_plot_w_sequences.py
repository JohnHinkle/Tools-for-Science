# Geochemistry Plot with sequences
# Plots geochemical data vs depth (mbsf)
# Adds light blue lines denoting detection of a desired probe (ex. ANME-1-350)
# Adds grey lines denoting no detection of desired probe, but sequencing attempted
...
# indicates depths where desired probe was detected
depths = []
[ax.axhline(y=i, linestyle='dotted', color='#1E88E5') for i in depths]

# indicates where no desired sequences were detected, but sequencing was attempted
fails = []
[ax.axhline(y=i, linestyle='dotted', color='grey') for i in fails]
...
# Labels the detection lines
# Mess around with the x and y values (ints) to make the labels look nice/properly spaced
plt.text(int, depths[0]+int, 'desired', fontweight='bold', color='darkblue')
plt.text(int, depths[1]+int, 'desired', fontweight='bold', color='darkblue')
plt.text(int, depths[2]+int, 'desired', fontweight='bold', color='darkblue')
plt.text(int, depths[3]+int, 'desired', fontweight='bold', color='darkblue')
